#AgentVerse/agentverse/output_parser/output_parser.py
...
@output_parser_registry.register("nlp_classroom_broadcast_9players")
class NlpClassroomBroadcast9PlayersParser(OutputParser):
    def parse(self, output: LLMResult) -> Union[AgentAction, AgentFinish]:
        text = output.content
        ...
        if action in ["Speak", "Advance"]:
            ...
            return AgentAction(action, result[0][1].strip() if result else "", text)

        if action == "Speak":
            return AgentFinish({"output": "[Speak] " + action_input}, text)
        elif action == "Advance":
            return AgentFinish({"output": "[Advance] " + action_input}, text)
        elif action == "Listen":
            return AgentFinish({"output": ""}, text)
...
@output_parser_registry.register("dhd_hypothesis")
@output_parser_registry.register("dhd_integrator")
class DHDPassthroughParser(OutputParser):
    """Pass-through parser for DHD hypothesizer / integrator agents.

    The agent's raw text is returned verbatim; the DiverseHypothesisRule does the
    structured field/answer extraction (see rules/dhd_parsing.py). This keeps the
    raw hypothesis cached for offline labeling and replay.
    """

    def parse(self, output: LLMResult) -> AgentFinish:
        return AgentFinish({"output": output.content}, output.content)


@output_parser_registry.register("dhd_recruiter")
class DHDRecruiterParser(OutputParser):
    """Tolerant recruiter parser for DHD.

    Parses a numbered roster ``N. <name> <sep> <description>`` where <sep> may be
    an ASCII hyphen, en-dash, em-dash, or colon (gpt-oss commonly emits en-dash).
    Falls back to name-only if no separator is present. Never raises on shortfall
    (the DiverseHypothesisRule fills any gap from the slots' static role
    descriptions), so a partial parse still yields whatever roles were found.
    """

    cnt_critic_agents: int = 0

    def parse(self, output: LLMResult) -> List[dict]:
        text = output.content or ""
        # numbered line, then name <sep> description; sep = - – — : (any spacing)
        pattern = re.compile(
            r"^\s*\d+[\.\)]\s*(.+?)\s*[\-–—:]\s*(.+?)\s*$",
            re.MULTILINE,
        )
        roles = pattern.findall(text)
        res = [{"name": n.strip(" *_."), "description": d.strip(" *_.")} for n, d in roles]
        if res:
            return res
        # Fallback: numbered lines with no separator -> use the whole line as desc.
        line_pat = re.compile(r"^\s*\d+[\.\)]\s*(.+?)\s*$", re.MULTILINE)
        names = line_pat.findall(text)
        return [{"name": f"Agent {i+1}", "description": n.strip(" *_.")}
...
@output_parser_registry.register("mgsm-critic-route")
class MGSMCriticRouteParser(OutputParser):
    """
    Like mgsm-critic-agree, but keeps the full text so the environment
    can parse routing tags from it.
    Supports:
      [Agree]
      [Route:Planner]
      [Route:Executor]
    ...
    def parse(self, output: LLMResult) -> AgentCriticism:
        ...
        if not text:
            raise OutputParserError("[empty critic output]")
        is_agree = "[Agree]" in text
        # keep full text always (so PerRule can parse [Route:...])
        return AgentCriticism(is_agree, text)

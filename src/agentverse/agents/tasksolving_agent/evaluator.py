# AgentVerse/agentverse/agents/tasksolving_agent/evaluator.py
...
class EvaluatorAgent(BaseAgent):
    max_history: int = 5
    current_debug_prompt_stage: str = "evaluation"
    current_failed_attempt_memory_size: int = 0
    current_candidate_revision_memory_size: int = 0
    ...
    async def astep(
        ...
        forced_history: Optional[List[dict]] = None,
    ) -> EvaluatorMessage:
        ...
        if forced_history is not None:
            history = list(forced_history)
        elif use_memory:
        ...
        self.log_prompt_budget(
            stage=str(getattr(self, "current_debug_prompt_stage", "evaluation")),
            prompt_token=prompt_token,
            history=history,
            extra_debug={
                "failed_attempt_ledger_size": int(
                    getattr(self, "current_failed_attempt_memory_size", 0) or 0
                ),
                "revision_ledger_size": int(
                    getattr(self, "current_candidate_revision_memory_size", 0) or 0
                ),
